...
- Adaptive weights: daily 4 AM optimizer uses return-weighted votes from **1-day daily-view** outcomes (`abs(price_change_pct)` scaled by each signal's magnitude share), tracks `analyst_score` and `insider_score`, and writes per-(sector, regime) weights when a pair has enough **daily views**; live scoring prefers (sector, regime) → (global, regime) → sector → global → defaults; majority regime label on the view's signals
- ML signal ensemble: LightGBM binary classifier (per-sector + global fallback) trained on **weighted-mean 6-vectors per daily view** (1-day `is_correct` labels) → ml_score, ml_direction, ml_confidence on each signal; disabled by default (ML_ENSEMBLE_ENABLED=true to activate). When validation_accuracy ≥ 55% (stored as percent, compared as ratio) and training_samples ≥ 50, ml_score is promoted into the composite as a gated 8% component (`has_ml=true`); otherwise it remains A/B comparison only
...
- Alembic migrations (18 revisions: 001–008, 010–019; 009 was never created)
...
- Total: 847 unit tests + 37 integration tests + 10 E2E tests
...
backend/           Python backend (FastAPI + Celery + SQLAlchemy)
  ...
  tests/           pytest test suite (847 unit tests + 37 integration tests)
...
| `backend/worker/utils/daily_aggregation.py` | Trading-date assignment, net-view score/conviction, feature aggregation, proportional credit |
...
Hourly (Celery Beat on Compute VM):
  ...
  :45 → evaluate signal outcomes and daily-view outcomes (1/3/5-day windows; learning uses 1-day views with conviction ≥ 0.20)
...
Daily:
  ...
  4:00 AM → compute adaptive signal weights (per-sector + per-regime, return-weighted 1-day daily views; includes analyst + insider)
  4:30 AM → train ML models (per-sector LightGBM from daily-view 1-day outcomes, if ML_ENSEMBLE_ENABLED)
...
1-day daily-view outcomes (each contributing signal votes with abs(price_change_pct)
scaled by its magnitude share of the net view). When a (sector, market_regime) pair
has enough daily views, those weights are preferred over sector/global. The optimizer
tracks analyst_score (default 0.07) and insider_score (default 0.08) alongside earnings
and options. RSI and trend weights are always written as 0.0 in DB (regime context
only, not additive). Views with |net_score| < 0.20 are stored but excluded from learning.
