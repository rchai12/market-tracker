...
- Adaptive weights: daily 4 AM optimizer uses return-weighted votes (`abs(price_change_pct)`), tracks `analyst_score` and `insider_score`, and writes per-(sector, regime) weights when a pair has enough samples; live scoring prefers (sector, regime) → (global, regime) → sector → global → defaults
- ML signal ensemble: LightGBM binary classifier (per-sector + global fallback) trained on component scores → ml_score, ml_direction, ml_confidence on each signal; disabled by default (ML_ENSEMBLE_ENABLED=true to activate). When validation_accuracy ≥ 55% (stored as percent, compared as ratio) and training_samples ≥ 50, ml_score is promoted into the composite as a gated 8% component (`has_ml=true`); otherwise it remains A/B comparison only
...
- Admin page: task triggers (scrape, seed, maintenance, outcomes, weights, options fetch, insider fetch, ML training), database stats table, ML model status, task failures list with retry, audit log
...
- SQLAlchemy models for all 30 tables
...
- Alembic migrations (17 revisions: 001–008, 010–018; 009 was never created)
...
- Total: 816 unit tests + 34 integration tests + 10 E2E tests
...
backend/           Python backend (FastAPI + Celery + SQLAlchemy)
  ...
  tests/           pytest test suite (816 unit tests + 34 integration tests)
...
Daily:
  ...
  4:00 AM → compute adaptive signal weights (per-sector + per-regime, return-weighted; includes analyst + insider)
...
sector/global. The optimizer tracks analyst_score (default 0.07) and insider_score (default 0.08) alongside earnings
