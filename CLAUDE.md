...
- Signal API: paginated list with direction/strength/ticker/sector filters, per-ticker history, latest signals feed, signal detail with outcomes + linked articles, accuracy trend + distribution endpoints, paginated daily views + today's predictions
...
- Redis caching layer: `@cached()` decorator factory on 6 endpoints (sector summary, trending, indicators, signal weights, today's predictions, DB stats), deterministic cache keys, SCAN-based invalidation, automatic cache-busting in Celery tasks after data updates
...
- Dashboard: Today's Predictions (daily net views, conviction-sorted, live change), signals feed (10 latest moderate+), clickable sector sentiment heatmap, top movers (bullish/bearish), article activity chart
